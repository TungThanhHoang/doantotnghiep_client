...
export default function Register({ changeStateModal }) {
  ...
  const { registerUser, loading, getLatLngLocation } = useContext(AuthContext);
  ...
  const handleRegisterForm = async (event) => {
    ...
    try {
      ...
      const sendData = await registerUser({ ...registerForm, city: "Đà Nẵng", distinct: selectDistinct, ward: selectWard, street: splitCharater.join(" "), username: `username${Math.random() * 1000}`, lat: getLatLng.Results[0].latitude, lng: getLatLng.Results[0].longitude });
          ...
          <form onSubmit={handleRegisterForm} className="flex flex-col overflow-y-auto md:flex-row">
            <div className="flex items-center justify-center mx-auto py-10">
              <div className="w-full">
                ...
                {loading
                  ? (
                    <button type="button" className="inline-flex items-center justify-center rounded-md p-2 px-10 font-medium mt-4 text-white text-md bg-yellow-500 hover:bg-yellow-600 " disabled>
                  ...
                  :
                  (
                    <button type="submit" className="rounded-md p-2 px-10 font-medium mt-4 text-white text-md bg-yellow-500 hover:bg-yellow-600 active:bg-yellow-700 focus:outline-none focus:ring focus:ring-yellow-300">
                      Đăng ký
                    </button>
                  )}
                <p className="mt-4">
