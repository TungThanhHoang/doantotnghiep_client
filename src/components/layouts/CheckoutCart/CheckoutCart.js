  ...
  const paymentStripe = async () => {
    ...
    const response = await checkoutByStripe({
      ...
      "data": {
        ...
        total: (totalPrice + DeliveryFee(Number(routes?.distance / 1000).toFixed(1), totalPrice) + calculatorPromotion())
      }
    })
    ...
    try {
      switch (checked) {
        ...
        case 2: paymentStripe()
          break;
        case 3: alert("Thanh toán metaMask")
          break;
        default:
          break;
      }
          ...
          <div className="card-info__user">
            ...
            <div className="detail-info">
              <div className="mt-4">
                ...
                <span className="name-user"> {lastName} {firstName}</span>
              </div>
              <div className="my-2">
                <span className="text-slate-600">Địa chỉ nhận hàng:</span>{" "}
                <span className="address-delivery font-medium text-slate-800">{street}, {ward}, {distinct}, Thành Phố Đà Nẵng</span>
              </div>
            ...
            <div>
              ...
              <div className="total-price__order">
                ...
                <span style={{ color: "var(--color-price)" }}>
                  {formatPrice.format(totalPrice + DeliveryFee(Number(routes?.distance / 1000).toFixed(1), totalPrice) + calculatorPromotion())}
                </span>
              </div>
            </div>
            {isLoadingBill ? (<button type="button" class="cursor-default inline-flex items-center justify-center h-12 w-full rounded-md p-2 px-10 font-medium mt-4 text-white text-md bg-green-500 " disabled>
              ...
              loading...
            </button>) : (<button className="bg-green-500 h-12 hover:bg-green-600">Đặt Mua</button>)}
          </form>
